...
exports.start = (config) ->
    ...
    conn.on_ "online", startPresenceTracking
    conn.on_ "stanza", (stanza) ->
        ...
        else if stanza.name == "message" and stanza.attrs.type != "error"
    ...
    switch presence.attrs.type
        ...
        when "probe"
            conn.send new xmpp.Element("presence",
                ...
                to: presence.attrs.from
                id: presence.attrs.id
            ).c("status").t("Small happy channel server!")
        when "error"
            # Error from a bare JID?
            unless resource
                # Remove all resources
                delete onlineResources[user]
            else if onlineResources[user]
                # Remove specific resource
                onlineResources[user] = onlineResources[user].filter((r) ->
                    r != resource
                )
                # No resources left?
        ...
        when "unavailable"
            ...
            unless onlineResources.hasOwnProperty(user)
...
getOnlineResources = (bareJid) ->
    (if onlineResources.hasOwnProperty(bareJid) then onlineResources[bareJid] else [])
